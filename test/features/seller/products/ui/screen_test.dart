...
void main() {
  ...
  // ── Builder 1: full screen body ─────────────────────────────────────────
  // Used for: loading, success, error, search tests
  Widget buildScreen() {
    return ScreenUtilInit(
      designSize: const Size(390, 844),
      minTextAdapt: true,
  ...
  testWidgets('tapping delete icon shows confirmation dialog', (tester) async {
    ...
    // Find the delete IconButton and get its center coordinates
    final deleteIcon = find.byIcon(Icons.delete);
    expect(deleteIcon, findsOneWidget);

    // tapAt uses raw screen coordinates — bypasses hit test boundaries
    final center = tester.getCenter(deleteIcon);
  ...
  testWidgets('tapping Cancel closes dialog without deleting', (tester) async {
    ...
    // Open dialog using tapAt
    final center = tester.getCenter(find.byIcon(Icons.delete));
    await tester.tapAt(center);
    await tester.pump();
