...
class OrderRepo {
  ...
  Future<ApiResult<OrderResponseModel>> placeOrder(
    String token,
  ...
  Future<ApiResult<PaymentInitiateResponseModel>> initiatePayment(
    String token,
    String orderId,
  ) async {
    try {
      final response = await apiService.paymentInitiate(
        "Bearer $token",
        PaymentInitiateRequestModel(orderId: orderId),
      );
      return ApiResult.success(response);
    } catch (e) {
      return ApiResult.failure(mapErrorToUserMessage(e));
